...
if (Test-Path -Path "C:\DFIR\CyberChef") {
    ...
    New-Item -Path "C:\DFIR\_Tools\Collections\CyberChef.lnk" -ItemType SymbolicLink -Target "C:\DFIR\CyberChef" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\CyberChef"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\Collections\CyberChef.lnk" -ItemType SymbolicLink -Target "C:\DFIR\CyberChef\CyberChef.exe" -Force
