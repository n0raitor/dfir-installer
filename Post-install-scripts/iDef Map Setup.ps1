...
if (Test-Path -Path "C:\DFIR\iDef Map Setup") {
    ...
    New-Item -Path "C:\DFIR\_Tools\Collections\iDef Map Setup.lnk" -ItemType SymbolicLink -Target "C:\DFIR\iDef Map Setup" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\iDef Map Setup"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\Collections\iDef Map Setup.lnk" -ItemType SymbolicLink -Target "C:\DFIR\iDef Map Setup\iDef Map Setup.exe" -Force
