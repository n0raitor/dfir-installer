...
if (Test-Path -Path "C:\DFIR\Bytecode-Viewer") {
    ...
    New-Item -Path "C:\DFIR\_Tools\Editors\Bytecode-Viewer.lnk" -ItemType SymbolicLink -Target "C:\DFIR\Bytecode-Viewer" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\Bytecode-Viewer"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\Editors\Bytecode-Viewer.lnk" -ItemType SymbolicLink -Target "C:\DFIR\Bytecode-Viewer\Bytecode-Viewer.exe" -Force
