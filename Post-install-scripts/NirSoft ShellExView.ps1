...
if (Test-Path -Path "C:\DFIR\NirSoft ShellExView") {
    ...
    New-Item -Path "C:\DFIR\_Tools\DEP\NirSoft ShellExView.lnk" -ItemType SymbolicLink -Target "C:\DFIR\NirSoft ShellExView" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\NirSoft ShellExView"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\DEP\NirSoft ShellExView.lnk" -ItemType SymbolicLink -Target "C:\DFIR\NirSoft ShellExView\NirSoft ShellExView.exe" -Force
