...
if (Test-Path -Path "C:\DFIR\MemoryBaseliner") {
    ...
    New-Item -Path "C:\DFIR\_Tools\Volatile\MemoryBaseliner.lnk" -ItemType SymbolicLink -Target "C:\DFIR\MemoryBaseliner" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\MemoryBaseliner"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\Volatile\MemoryBaseliner.lnk" -ItemType SymbolicLink -Target "C:\DFIR\MemoryBaseliner\MemoryBaseliner.exe" -Force
