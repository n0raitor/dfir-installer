...
if (Test-Path -Path "C:\DFIR\sort-PhotorecRecoveredFiles") {
    ...
    New-Item -Path "C:\DFIR\_Tools\Carving\sort-PhotorecRecoveredFiles.lnk" -ItemType SymbolicLink -Target "C:\DFIR\sort-PhotorecRecoveredFiles" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\sort-PhotorecRecoveredFiles"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\Carving\sort-PhotorecRecoveredFiles.lnk" -ItemType SymbolicLink -Target "C:\DFIR\sort-PhotorecRecoveredFiles\sort-PhotorecRecoveredFiles.exe" -Force
