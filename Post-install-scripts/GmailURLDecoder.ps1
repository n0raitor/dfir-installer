...
if (Test-Path -Path "C:\DFIR\GmailURLDecoder") {
    ...
    New-Item -Path "C:\DFIR\_Tools\Email Tools\GmailURLDecoder.lnk" -ItemType SymbolicLink -Target "C:\DFIR\GmailURLDecoder" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\GmailURLDecoder"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\Email Tools\GmailURLDecoder.lnk" -ItemType SymbolicLink -Target "C:\DFIR\GmailURLDecoder\GmailURLDecoder.exe" -Force
