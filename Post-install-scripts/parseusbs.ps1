...
if (Test-Path -Path "C:\DFIR\parseusbs") {
    ...
    New-Item -Path "C:\DFIR\_Tools\USB Forensics\parseusbs.lnk" -ItemType SymbolicLink -Target "C:\DFIR\parseusbs" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\parseusbs"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\USB Forensics\parseusbs.lnk" -ItemType SymbolicLink -Target "C:\DFIR\parseusbs\parseusbs.exe" -Force
