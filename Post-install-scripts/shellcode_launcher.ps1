...
if (Test-Path -Path "C:\DFIR\shellcode_launcher") {
    ...
    New-Item -Path "C:\DFIR\_Tools\SRE\shellcode_launcher.lnk" -ItemType SymbolicLink -Target "C:\DFIR\shellcode_launcher" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\shellcode_launcher"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\SRE\shellcode_launcher.lnk" -ItemType SymbolicLink -Target "C:\DFIR\shellcode_launcher\shellcode_launcher.exe" -Force
