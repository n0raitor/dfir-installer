...
if (Test-Path -Path "C:\DFIR\NirSoft DownTester") {
    ...
    New-Item -Path "C:\DFIR\_Tools\DEP\NirSoft DownTester.lnk" -ItemType SymbolicLink -Target "C:\DFIR\NirSoft DownTester" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\NirSoft DownTester"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\DEP\NirSoft DownTester.lnk" -ItemType SymbolicLink -Target "C:\DFIR\NirSoft DownTester\NirSoft DownTester.exe" -Force
