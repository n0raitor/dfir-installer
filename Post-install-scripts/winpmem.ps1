...
if (Test-Path -Path "C:\DFIR\winpmem") {
    ...
    New-Item -Path "C:\DFIR\_Tools\Volatile\winpmem.lnk" -ItemType SymbolicLink -Target "C:\DFIR\winpmem" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\winpmem"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\Volatile\winpmem.lnk" -ItemType SymbolicLink -Target "C:\DFIR\winpmem\winpmem.exe" -Force
