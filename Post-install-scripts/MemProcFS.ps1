...
if (Test-Path -Path "C:\DFIR\MemProcFS") {
    ...
    New-Item -Path "C:\DFIR\_Tools\Volatile\MemProcFS.lnk" -ItemType SymbolicLink -Target "C:\DFIR\MemProcFS" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\MemProcFS"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\Volatile\MemProcFS.lnk" -ItemType SymbolicLink -Target "C:\DFIR\MemProcFS\MemProcFS.exe" -Force
