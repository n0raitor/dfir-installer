...
if (Test-Path -Path "C:\DFIR\NirSoft IPNetInfo") {
    ...
    New-Item -Path "C:\DFIR\_Tools\DEP\NirSoft IPNetInfo.lnk" -ItemType SymbolicLink -Target "C:\DFIR\NirSoft IPNetInfo" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\NirSoft IPNetInfo"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\DEP\NirSoft IPNetInfo.lnk" -ItemType SymbolicLink -Target "C:\DFIR\NirSoft IPNetInfo\NirSoft IPNetInfo.exe" -Force
