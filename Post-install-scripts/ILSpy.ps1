...
if (Test-Path -Path "C:\DFIR\ILSpy") {
    ...
    New-Item -Path "C:\DFIR\_Tools\Utilities\ILSpy.lnk" -ItemType SymbolicLink -Target "C:\DFIR\ILSpy" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\ILSpy"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\Utilities\ILSpy.lnk" -ItemType SymbolicLink -Target "C:\DFIR\ILSpy\ILSpy.exe" -Force
